...
import CamTimes from '../components/CamTimes2'
import Center from '../components/Center'
import { bitsToForm, formToBits } from '../lib/timetable'
...
const CamTimesContainer = ({
  ...
  if (
    ...
    return (
      <CamTimes
        values={bitsToForm(
          shiftSchedule(cameraTimes.camera.timeZone)(schedules),
        )}
        onChange={newFormValues => {
          const unshiftSchedule = shiftSchedule(
            -1 * cameraTimes.camera.timeZone,
          )
          return onChange(unshiftSchedule(formToBits(newFormValues)))
        }}
        {...props}
      />
    )
